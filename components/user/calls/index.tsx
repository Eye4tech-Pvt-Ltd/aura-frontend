...
import { Phone, PhoneIncoming, PhoneOutgoing, PhoneMissed, Search, Filter, X, Play, Download, ThumbsUp, ThumbsDown, User } from 'lucide-react';
...
const calls = [
  { 
    ...
    outcome: 'Booked', 
    sentiment: 'positive',
    transcript: 'Customer called to schedule a dental cleaning appointment. Discussed available time slots and confirmed booking for next Tuesday at 2 PM.',
    intents: ['appointment_booking', 'service_inquiry'],
    aiSummary: 'Successfully booked dental cleaning appointment for Sarah Johnson on Tuesday, Jan 23 at 2:00 PM. Customer was satisfied with the available time slot.'
  },
  { 
    id: 2, 
    caller: 'Michael Chen', 
    ...
    outcome: 'Info', 
    sentiment: 'neutral',
    transcript: 'Customer inquired about office hours and services offered. Provided information about business hours and service packages.',
    intents: ['hours_inquiry', 'service_inquiry'],
    aiSummary: 'Provided business information to Michael Chen. Shared office hours (Mon-Fri 9AM-6PM) and overview of available services.'
  },
  { 
    id: 3, 
    ...
    outcome: 'Booked', 
    sentiment: 'positive',
    transcript: 'Customer called regarding follow-up appointment. Discussed treatment plan and scheduled next visit.',
    intents: ['appointment_booking', 'follow_up'],
    aiSummary: 'Scheduled follow-up appointment for Emma Williams on January 28 at 11:00 AM. Confirmed treatment plan details.'
  },
  { 
    id: 4, 
    caller: 'Unknown', 
    ...
    transcript: '',
    intents: [],
    aiSummary: 'Missed call - no voicemail left. Consider adding to callback queue.'
  },
];
                ...
                >
                  <div className="flex items-start gap-3">
                    <div className={`p-2 rounded-lg shadow-sm ${
                      call.type === 'inbound' ? 'bg-gradient-to-br from-primary-400 to-primary-600' :
                      call.type === 'outbound' ? 'bg-gradient-to-br from-accent-green-400 to-accent-green-600' :
                      'bg-gradient-to-br from-accent-red-400 to-accent-red-600'
                    }`}>
                      {call.type === 'inbound' ? <PhoneIncoming className="w-4 h-4 text-white" /> :
                       call.type === 'outbound' ? <PhoneOutgoing className="w-4 h-4 text-white" /> :
                       <PhoneMissed className="w-4 h-4 text-white" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <h3 className="font-semibold text-slate-900 truncate">{call.caller}</h3>
                        <Badge 
                          variant={
                            call.outcome === 'Booked' ? 'success' : 
                            call.outcome === 'Missed' ? 'error' : 
                            'neutral'
                          }
                          size="sm"
                        >
          ...
          <div className="bg-white rounded-xl border border-slate-200/60 shadow-md p-6 max-h-[calc(100vh-200px)] overflow-y-auto">
            ...
            <div className="mb-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h3 className="font-semibold text-slate-900">{selectedCall.caller}</h3>
                  <p className="text-sm text-slate-600">{selectedCall.number}</p>
                </div>
              </div>
              ...
              <div className="bg-slate-50 rounded-lg p-4 border border-slate-200 mb-4">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <span className="text-sm font-medium text-slate-700">{selectedCall.duration}</span>
                </div>
                <div className="flex items-center gap-2">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div>
                  ...
                  <p className="font-medium text-slate-900">{selectedCall.time}</p>
                </div>
                <div>
                  <p className="text-slate-600 mb-1">Duration</p>
                  <p className="font-medium text-slate-900">{selectedCall.duration}</p>
                </div>
                <div>
                  <p className="text-slate-600 mb-1">Outcome</p>
                  <Badge variant={selectedCall.outcome === 'Booked' ? 'success' : 'neutral'}>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="font-medium text-slate-900 capitalize">{selectedCall.sentiment}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            {selectedCall.aiSummary && (
              <div className="mb-6">
                ...
                <div className="bg-gradient-to-r from-accent-purple-50 to-accent-purple-100 rounded-lg p-4 border border-accent-purple-200">
                  <p className="text-sm text-slate-700">{selectedCall.aiSummary}</p>
                </div>
              </div>
            )}
            ...
            {selectedCall.intents.length > 0 && (
              <div className="mb-6">
                <h3 className="font-semibold text-slate-900 mb-3">Detected Intents</h3>
